...
def getImgSlice(frame, centerPt):
	'''
	Get a verticle slice of the image around a central point
	'''
...
def frameSubtractionDetection(prevFrame, newFrame, centerPt):
	...
	# Check each user's centerpoint for gross movement
...
def main():
	...
	phaseInterval = triangular(.5,1.5, 1.2) * 2    
	phaseStart = clock()
	...
	while True:
		...
		while startGame == -1:
			success, frame = camera.read()
			startGame = cv2.waitKey(1)
			if startGame != -1:
				cv2.destroyAllWindows()
				break
			cv2.imshow("Begin!", frame)

		# Change phase if previous phase has timed out
		if clock() >= phaseInterval + phaseStart:
			red = not red
			phaseInterval = triangular(1,1.5, 1.8)  
			phaseStart = clock()
			if red:
				display[:][:] = (0,0,255)
			else:
		...
		# Always need to know who your players are, even in greenlight phase
		...
		# If in red phase check for motion
		if red and clock() > phaseStart + phaseInterval / 10:
			for user in userList:
				if checkRelevance(user):
					...
					user.isOut = inMotion
					if user.isOut:
						cv2.imshow("You're Out!", getImgSlice(frame, user.massCenter))
